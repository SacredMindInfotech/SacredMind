                                        ...
                                        {jobs.length > 0 ? (
                                            jobs.map((job: Job) => (
                                                <div key={job.id} className="border border-gray-900 p-6">
                                                    ...
                                                    <p className="text-gray-600 mb-4">{job.location === "REMOTE" ? "Remote" : job.location === "HYBRID" ? "Hybrid" : "On-Site"} | {job.type === "FULL_TIME" ? "Full Time" : job.type === "PART_TIME" ? "Part Time" : job.type === "CONTRACT" ? "Contract" : "Internship"} | {job.experience} | {job.education}</p>
                                                    <button onClick={() => { router(`/careers/${job.id}`) }} className="px-6 py-3 rounded-md border border-white bg-gray-900 text-white hover:shadow-[4px_4px_0px_0px_rgba(0,0,0)] hover:text-black hover:border-gray-900 hover:bg-white transition duration-200 montserrat-secondary cursor-pointer whitespace-nowrap">
                                                        View Job
                                                    </button>
                                                </div>
                                            ))
